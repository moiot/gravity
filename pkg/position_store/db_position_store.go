...
func (s *dbPositionStore) Stage() config.InputMode {
	return s.stage
}
...
func (s *dbPositionStore) loadPosition() {
	err := retry.Do(func() error {
		...
		if err == sql.ErrNoRows {
			...
			s.stage = config.InputMode(stage)
...
func (s *dbPositionStore) Position() Position {
	s.RLock()
	defer s.RUnlock()
...
func (s *dbPositionStore) Update(pos Position) {
	s.Lock()
	defer s.Unlock()
